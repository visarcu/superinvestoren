...
async function run() {
  ...
  for (const [slug, cik] of Object.entries(investorCiks)) {
    ...
    try {
      ...
      // 2) Nur echte Quarterly-Reports (13F-HR), keine Amendments (HR/A)
      let filings = hist
      ...
      const seen = new Set()
      for (const { date, period, txtHref, xmlHref } of filings) {
        ...
        // 4) XML oder TXT laden
        let raw = ''
        try {
          raw = await fetchUrl(xmlHref)
        } catch {
          console.warn(`  ↪ XML fehlgeschlagen, versuche TXT für ${slug} ${key}`)
          try {
