...
const TOOLTIP_W = 320
const TOOLTIP_H = 200  // Schätzung für Clamp-Berechnung
const GAP = 14          // Abstand zwischen Element und Tooltip
...
export function ProductTour() {
  ...
  const [visible, setVisible]     = useState(false)   // für fade-in nach Navigation
  const rafRef  = useRef<number | null>(null)
  const retryRef = useRef<ReturnType<typeof setTimeout> | null>(null)

  const step = TOUR_STEPS[stepIdx]

  // ── Init: Tour starten falls noch nicht abgeschlossen ──────────────────────
  useEffect(() => {
    ...
    if (isActive) {
      ...
      // Kurze Verzögerung damit die App vollständig gerendert ist
      const t = setTimeout(() => {
  ...
  // ── Navigation zur richtigen Route wenn Step sich ändert ───────────────────
  useEffect(() => {
  ...
  // ── Element tracken + Spotlight/Tooltip positionieren ─────────────────────
  ...
  useEffect(() => {
    ...
    // Erste Attempt nach kurzer Pause (für Seitenübergang)
  ...
  // Spotlight beim Scrollen/Resize nachführen
  useEffect(() => {
  ...
  return (
    <>
      {/* ── Halbtransparentes Overlay mit Spotlight-Ausschnitt ────────────── */}
      ...
      {/* ── Highlight-Ring um das Ziel-Element ───────────────────────────── */}
      {spotlight && visible && (
        <div
