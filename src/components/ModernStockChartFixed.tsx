// src/components/ModernStockChartFixed.tsx - PROPERLY FIXED VERSION
'use client'
...
const ModernStockChart: React.FC<Props> = ({ ticker, data, onAddComparison }) => {
  ...
  const getThemeColors = () => {
    ...
    return {
      ...
      areaTopColor: isDark ? 'rgba(74, 222, 128, 0.3)' : 'rgba(5, 150, 105, 0.2)',
      areaBottomColor: isDark ? 'rgba(74, 222, 128, 0.05)' : 'rgba(5, 150, 105, 0.02)',
    }
  }

  // Get filtered data based on time range - FIXED VERSION
  const getFilteredData = () => {
    ...
    console.log(`📊 Raw data: ${data.length} items, range: ${selectedRange}`)
    
    // Sort all data by date first - CRITICAL FIX
    const sortedData = [...data].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
    
    if (selectedRange === 'MAX') {
      console.log(`✅ MAX range: returning all ${sortedData.length} items`)
      return sortedData
    }
    ...
    if (selectedRange === 'YTD') {
      ...
      console.log(`📅 YTD cutoff: ${cutoffDate.toISOString().split('T')[0]}`)
    } else {
      const range = TIME_RANGES.find(r => r.label === selectedRange)
      if (!range || typeof range.days !== 'number') {
        console.log(`❌ Invalid range: ${selectedRange}`)
        return sortedData
      }
      
      cutoffDate = new Date()
      cutoffDate.setDate(cutoffDate.getDate() - range.days)
      console.log(`📅 ${selectedRange} cutoff: ${cutoffDate.toISOString().split('T')[0]} (${range.days} days ago)`)
    }
    ...
    console.log(`✅ Filtered ${selectedRange}: ${filtered.length} items from ${sortedData.length} total`)
  ...
  useEffect(() => {
    ...
    if (!filteredData.length) {
      console.log(`❌ No filtered data for range ${selectedRange}`)
    ...
    // Convert data to chart format - FIXED FORMAT
    const chartData = filteredData.map(d => ({
      time: d.date, // YYYY-MM-DD format works directly
      value: d.close,
    }))

    console.log(`📊 Setting chart data: ${chartData.length} points`)
    console.log(`📊 Sample data:`, chartData.slice(0, 3), '...', chartData.slice(-3))
    ...
    // Fit content with proper timing
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <div className={`text-xs mt-2 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              {filteredDataCount} Datenpunkte für {selectedRange}
            </div>
          </div>
