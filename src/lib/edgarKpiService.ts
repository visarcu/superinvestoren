...
export const PILOT_COMPANIES: Record<string, CompanyConfig> = {
  ...
  NOW: {
    ...
    kpiHints: [
      ...
      'NET NEW customers with more than $1 million in ACV added in THIS quarter (ServiceNow reports "we had X deals over $1M in ACV in Q4", typical range 80-250 per quarter). Store as count. metric name "new_1m_acv_deals", label "Net New $1M+ ACV Deals". If only a cumulative total is given, SKIP.',
      'TOTAL cumulative customers with more than $20 million in ACV (typical range: 50-150). Store as count. metric name "customers_20m_acv", label "Customers $20M+ ACV". Only extract if explicitly stated as a cumulative total, not net new.',
      'NOTE: Do NOT extract "renewal rate" — ServiceNow no longer publishes a specific percentage in the press release.',
    ],
  },
  // ── Streaming / Media ─────────────────────────────────────────────────────
  DIS: {
  ...
  ETSY: {
    ...
    kpiHints: [
      ...
      'consolidated take rate (revenue / GMS, typically 21-24% for Etsy) — store as percent, metric "take_rate", label "Take Rate"',
