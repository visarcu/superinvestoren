...
type Tab = 'overview' | 'news' | 'financials' | 'earnings' | 'kpis' | 'ai'
          ...
          <div className="w-full max-w-4xl">
            {aiAnalysis ? (
              <div className="bg-[#0c0c16] border border-white/[0.04] rounded-2xl p-6">
                ...
                <div className="[&_h2]:text-[14px] [&_h2]:font-semibold [&_h2]:text-white/60 [&_h2]:mt-5 [&_h2]:mb-2
                  ...
                  {aiAnalysis.split('\n').map((line, i) => {
                    ...
                    if (line.startsWith('- **')) {
                      ...
                      if (m) return <p key={i}><strong>{m[1]}</strong>{m[2] ? `: ${m[2]}` : ''}</p>
                    }
                    if (line.startsWith('- ')) return <p key={i} className="pl-3 border-l-2 border-violet-500/10">{line.replace('- ', '')}</p>
                    if (line.trim() === '') return null
              ...
              <div className="text-center py-20">
                ...
                <p className="text-white/12 text-xs mt-1 mb-4">Basierend auf SEC Finanzdaten, Earnings, Insider-Trades & Superinvestoren</p>
                <button
                  onClick={() => {
                    setAiLoading(true)
