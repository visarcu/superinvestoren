...
// Fetch growth data from our existing Growth API
async function getGrowthData(symbol: string): Promise<any> {
  try {
    const baseUrl = process.env.NEXTAUTH_URL || process.env.VERCEL_URL || 'http://localhost:3000'
    const response = await fetch(`${baseUrl}/api/growth/${symbol}`, {
      next: { revalidate: 21600 } // Cache für 6 Stunden
    })
    
    if (!response.ok) {
      console.log(`⚠️ Growth data not available for ${symbol}`)
      return {}
    }
    ...
    console.error(`❌ Error fetching growth data for ${symbol}:`, error)
    return {}
  }
}

// Fetch financial ratios from our existing Scores API
async function getFinancialRatios(symbol: string): Promise<any> {
  try {
    const baseUrl = process.env.NEXTAUTH_URL || process.env.VERCEL_URL || 'http://localhost:3000'
    const response = await fetch(`${baseUrl}/api/scores/${symbol}`, {
      next: { revalidate: 21600 } // Cache für 6 Stunden
    })
    
    if (!response.ok) {
      console.log(`⚠️ Financial ratios not available for ${symbol}`)
      return {}
    }
    ...
    return {
      roe: breakdown.profitability?.metrics?.roe || null,
      currentRatio: breakdown.safety?.metrics?.currentRatio || null,
      debtToEquity: breakdown.safety?.metrics?.debtToEquity || null,
      grossMargin: breakdown.profitability?.metrics?.grossMargin || null,
      netMargin: breakdown.profitability?.metrics?.netMargin || null,
      operatingMargin: null, // Not available in this structure
      roic: null // Not available in this structure
    }
  } catch (error) {
...
// Quality Value screener using our stocks database and Scores API
async function runQualityValueScreener(searchParams: URLSearchParams, isPreview = false): Promise<EnhancedStock[]> {
  const minROE = parseFloat(searchParams.get('roeMoreThan') || '0')
  const minCurrentRatio = parseFloat(searchParams.get('currentRatioMoreThan') || '0')
  const maxDebtToEquity = parseFloat(searchParams.get('debtToEquityLowerThan') || '999')
  ...
  console.log('🎯 Running Quality Value Screener with our financial data APIs')
  console.log(`📊 Criteria: ROE>${minROE}%, CurrentRatio>${minCurrentRatio}, DebtEquity<${maxDebtToEquity}, PE:${minPE}-${maxPE}, MarketCap>${minMarketCap}`)
  ...
  const validTickers = stocks
    ...
    .slice(0, 200) // Limit to 200 for fast screening

  console.log(`📊 Screening ${validTickers.length} stocks for quality value metrics`)

  const batchSize = 15 // Smaller batch size for multiple API calls
  ...
  for (let i = 0; i < validTickers.length; i += batchSize) {
    ...
    try {
      ...
      // Get detailed financial ratios for filtered stocks
      for (const stock of filteredQuotes) {
        const ratiosData = await getFinancialRatios(stock.symbol)
        
        // Apply quality value filters (ROE in API is decimal, convert to percentage)
        const roePercent = ratiosData.roe ? ratiosData.roe * 100 : 0
        if (minROE > 0 && roePercent < minROE) continue
        if (minCurrentRatio > 0 && (!ratiosData.currentRatio || ratiosData.currentRatio < minCurrentRatio)) continue
        if (maxDebtToEquity < 999 && (!ratiosData.debtToEquity || ratiosData.debtToEquity > maxDebtToEquity)) continue
...
async function runProfitableGrowthScreener(searchParams: URLSearchParams, isPreview = false): Promise<EnhancedStock[]> {
  ...
  const validTickers = stocks
    ...
    .slice(0, 150) // Limit to 150 for fast dual API screening
...
async function runMegaCapsScreener(searchParams: URLSearchParams, isPreview = false): Promise<EnhancedStock[]> {
  ...
  const remainingTickers = allValidTickers.filter(ticker => !priorityMegaCaps.includes(ticker)).slice(0, 500)
...
async function runDefensiveQualityScreener(searchParams: URLSearchParams, isPreview = false): Promise<EnhancedStock[]> {
  ...
  const remainingTickers = allValidTickers.filter(ticker => !defensiveCompanies.includes(ticker)).slice(0, 50)
  const validTickers = [...priorityTickers, ...remainingTickers]
...
async function runTechGrowthScreener(searchParams: URLSearchParams, isPreview = false): Promise<EnhancedStock[]> {
  ...
  const remainingTickers = allValidTickers.filter(ticker => !techCompanies.includes(ticker)).slice(0, 200)
  const validTickers = [...priorityTickers, ...remainingTickers]
...
async function runGrowthScreener(searchParams: URLSearchParams, isPreview = false): Promise<EnhancedStock[]> {
  ...
  for (let i = 0; i < validTickers.length; i += batchSize) {
    ...
    try {
      ...
      // Get growth data for filtered stocks
      for (const stock of filteredQuotes) {
        const growthData = await getGrowthData(stock.symbol)
        ...
        results.push({
          ...
          epsGrowth3Y: growthData.epsGrowth3Y,
          epsGrowth1Y: growthData.epsGrowth1Y,
          ebitdaGrowth3Y: growthData.ebitdaGrowth3Y,
          fcfGrowth3Y: growthData.fcfGrowth3Y,
