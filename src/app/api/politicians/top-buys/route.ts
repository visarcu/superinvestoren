...
// Quelle: eigene DB (Tabelle PoliticianTrade), befüllt vom Cron
...
function parseAmountRange(amount: string): { min: number; max: number } {
  ...
  const single = clean.match(/over(\d+)/i)
  if (single) {
...
export async function GET() {
  try {
    ...
    for (const trade of trades) {
      if (trade.type !== 'purchase') continue
      ...
      if (trade.politicianName) entry.politicians.add(trade.politicianName)
